...
def SegmentScan(lines, segmentSize=0.5):
    ...
    for line in lines:
        ...
        if (vals[0]=='0' and vals[4]=='1'):
            ...
            # While the segment still needs to be cut; the tolerance stops
            # float drift from leaving a zero-length sliver when the raster
            # length is an exact multiple of segmentSize (e.g. 10 mm / 0.1)
            while (rasterDistance>segmentSize + 1e-9):
